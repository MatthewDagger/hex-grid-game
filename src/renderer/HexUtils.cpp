...
// Default two orientations for ease of access
const Orientation orientation_point
...
void draw_tile(Tile t, Layout layout) {
    const std::vector<Vector2> corners = polygon_corners(layout, t.location);

    for (int i = 0; i < 6; ++i) {
        DrawLineV(corners[i], corners[(i + 1) % 6], DARKBLUE);
    }
}